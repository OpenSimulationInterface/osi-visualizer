...
OsiParser::ParseReceivedSDMessage(const osi3::SensorData& sd)
{
	...
	ParseSensorData(sd, objectMessage, laneMessage, pointMessage);

	emit MessageParsed(objectMessage, laneMessage, pointMessage);
}
	...
	PointMessage& pointMessage)
{
	...
	if (sensorData.has_feature_data()) {
		// Parse Lidar Point Cloud
		for (int i = 0; i < sensorData.feature_data().lidar_sensor_size(); i++) {
			const osi3::LidarDetectionData& ldd = sensorData.feature_data().lidar_sensor().Get(i);
			for (int j = 0; j < sensorData.feature_data().lidar_sensor().Get(i).detection_size(); j++) {
				osi3::Spherical3d pos = ldd.detection().Get(j).position();
